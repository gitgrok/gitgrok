...
import { searchFinished, searchInitFinished, searchStarted } from "./search-state.actions";

@Injectable()
export class SearchStateEffects {
    constructor(private readonly actions$: Actions, private readonly searchService: SearchService, private readonly store: Store) { }

    executeSearch$ = createEffect(() =>
        this.actions$.pipe(
            ofType(searchStarted),
            switchMap(({ query , pathFilter}) => this.searchService.getV2(query, pathFilter)),
