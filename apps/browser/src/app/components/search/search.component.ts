...
import { getResults, getSelectedRepos } from '../../state/search/search-state.selectors';
...
export class SearchComponent {
  ...
  private startSearch(repos, searchFormValue) {
    return this.store.dispatch(searchStarted({ repos: searchFormValue.repoFilter ? repos.filter(rep => rep.includes(searchFormValue.repoFilter)) : repos, query: searchFormValue.query, pathFilter: searchFormValue.pathFilter }))
  }
}
